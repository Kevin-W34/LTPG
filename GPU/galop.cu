...
public:
    ...
    void new_neworder(int Loc)
    {
        ...
        set_length.push_back(5 + 3 * query.O_OL_CNT);
        TID_global += 1;
    }
    ...
    void sort_v_and_id()
    {
        ...
        for (size_t i = 1; i < set_loc_vec.size(); i++)
        {
            ...
            {
                int tid_pre = set_TID_list[i - 1];
                int tid_cur = set_TID_list[i];
                int length_pre = 0;
                int length_cur = 0;
                for (size_t ii = 0; ii < tid_cur; ii++)
                {
                    length_cur += set_length[ii];
                }
                for (size_t ii = 0; ii < tid_pre; ii++)
                {
                    length_pre += set_length[ii];
                }
                int tmp = 0;
                for (size_t ii = 0; ii < set_length[tid_pre]; ii++)
                {
                    if (set_vec[length_pre + ii].Loc == set_loc_list[i])
                    {
                        tmp = length_pre + ii;
                    }
                }
                for (size_t jj = 0; jj < set_length[tid_cur]; jj++)
                {
                    if (set_loc_list[i] == set_vec[length_cur + jj].Loc)
                    {
                        set_vec[length_cur + jj].pre_Loc_in_set_Loc_vec = tmp;
                        if (length_cur + jj != tmp)
                        {
                            break;
                        }
                    }
    ...
    __device__ void step_3(int size)
    {
        ...
        if (thID < size) // set_TID_d size
        {
            // if (thID > 1)
            // {
            //     int pre = set_loc_d[thID - 1];
            //     int cur = set_loc_d[thID];
            //     if (pre == cur)
            //     {
            //         pre = set_TID_list[thID - 1];
            //         cur = set_TID_list[thID];
            //         int length_pre = 0;
            //         int length_cur = 0;
            ...
            //             length_cur += set_length[ii];
            //         }
            //         for (size_t ii = 0; ii < pre; ii++)
            //         {
            //             length_pre += set_length[ii];
            //         }
            //         int tmp = 0;
            //         for (size_t ii = 0; ii < set_length[pre]; ii++)
            //         {
            //             if (set_vec[length_pre + ii].Loc == set_loc_list[i])
            //             {
            //                 tmp = length_pre + ii;
            //             }
            //         }
            //         for (size_t jj = 0; jj < set_length[cur]; jj++)
            //         {
            //             if (set_loc_list[thID] == set_vec[length_cur + jj].Loc)
            //             {
            //                 set_vec[length_cur + jj].pre_Loc_in_set_Loc_vec = tmp;
            //                 if (length_cur + jj != tmp)
            //                 {
            ...
            int pre_loc = set_d[thID].pre_Loc_in_set_Loc_vec;
            int pre_state = set_d[pre_loc].state;

            bool blocked = true;
            while (blocked)
            {
                if (pre_state == 1 && thID == 0)
                {
...
__global__ void kernel_3(Transaction T, int size)
{
    T.step_3(size);
}
...
int main(int argc, char const *argv[])
{
    ...
    for (size_t m = 0; m < EPOCH; m++)
    {
        ...
        //     cudaDeviceSynchronize();
        // }
        kernel_3<<<(T.set_TID_vec.size() / BLOCKSIZE + 1), BLOCKSIZE>>>(T, T.set_TID_vec.size());
        cudaDeviceSynchronize();
        time_t end = clock();
