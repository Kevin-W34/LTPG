...
public:
    ...
    /// @brief clear conflict log in the last batch
    void clear_LOG()
    {
        cudaMemset(log->LOG_WAREHOUSE_R, 0xffffffff, sizeof(unsigned int) * WAREHOUSE_SIZE * 32);
        cudaMemset(log->LOG_WAREHOUSE_W, 0xffffffff, sizeof(unsigned int) * WAREHOUSE_SIZE * 32);
        cudaMemset(log->LOG_DISTRICT_R, 0xffffffff, sizeof(unsigned int) * DISTRICT_SIZE * 32);
        cudaMemset(log->LOG_DISTRICT_W, 0xffffffff, sizeof(unsigned int) * DISTRICT_SIZE * 32);
    ...
    /// @brief create history table, only used for write data
    void create_history(unsigned int thID)
    {
    }

    /// @brief create neworder table, only used for write data
