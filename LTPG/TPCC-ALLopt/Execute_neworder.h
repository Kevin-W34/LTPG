...
/// op_a_b means step b of operation a
...
namespace NEWORDER_NAMESPACE
{
    ...
    /// @param neworder_set      local read/write set
    /// @param neworder_query    query info
    /// @param log
    /// @param snapshot
    /// @param index
    ///
    ...
    /// read the W_TAX (column 1) with KEY (W_ID) from warehouse_snapshot to local set
    /// select warehouse
    ///
    ...
    /// regist the Read operation on the data item with KEY (D_W_ID , D_ID) to table LOG_DISTRICT_R
    /// select district
    ///
    ...
    /// read the D_TAX (column 2) and D_NEXT_O_ID (column 4) with KEY (D_W_ID , D_ID) from district_snapshot to local set
    ///
    /// select warehouse
    ///
    ...
    /// regist the Read operation on the data item with KEY (C_W_ID, C_D_ID, C_ID) to table LOG_CUSTOMER_R
    /// select customer
    ///
    ...
    /// read the C_DISCOUNT (column 4) and C_CREDIT (column 10) and C_LAST (column 14) with KEY (C_W_ID, C_D_ID, C_ID) from customer_snapshot to local set
    ///
    /// select customer
    ///
    ...
    /// regist the insert operation on the data item with KEY (NO_W_ID, NO_D_ID, NO_O_ID) to table LOG_NEWORDER_R
    /// insert neworder
    ///
    ...
    /// insert(write) the NO_O_ID (column 0) and NO_D_ID (column 1) and NO_W_ID (column 2) with KEY  (NO_W_ID, NO_D_ID, NO_O_ID) from neworder_snapshot to local set
    ///
    /// insert neworder
    ///
    ...
    /// regist the insert operation on the data item with KEY (O_W_ID, O_D_ID, O_ID) to table LOG_ORDER_R
    /// insert order
    ///
    ...
    /// insert(write) the O_ID (column 0) and O_D_ID (column 1) and O_W_ID (column 2) with KEY  (O_W_ID, O_D_ID, O_ID) from neworder_snapshot to local set
    ///
    /// insert order
    ///
    ...
    /// regist the read operation on the data item with KEY (I_ID) to table LOG_ITEM_R
    ///
    /// select item. Each transaction needs to be executed o_ol_cnt times, once per warp.   ( o_ol_cnt [5,15] )
    ///
    ...
    /// read the I_PRICE (column 2) and I_NAME (column 3) and I_DATA (column 4) with KEY (I_ID) from item_snapshot to local set
    ///
    /// select item
    ///
    ...
    /// regist the update operation on the data item with KEY (S_W_ID, S_I_ID) to table LOG_STOCK_R
    ///
    /// update stock. Each transaction needs to be executed o_ol_cnt times, once per warp.   ( o_ol_cnt [5,15] )
    ///
    ...
    /// update(write) the S_QUANTITY (column 2) and S_YTD (column 3) and S_ORDER_CNT (column 4) with KEY (S_W_ID, S_I_ID)  simulate reading and writing data
    ///
    /// simulate the reading and writing of data, and omit the calculation process of data
    /// update stock
    ///
    ...
    /// regist the insert operation on the data item with KEY (OL_W_ID, O_OL_ID , ID) to table LOG_STOCK_R
    ///
    /// insert orderline. Each transaction needs to be executed o_ol_cnt times, once per warp.   ( o_ol_cnt [5,15] )
    ///
    ...
    /// insert(write) the OL_O_ID (column 1) and OL_D_ID (column 2) and OL_W_ID (column 3) with KEY  (OL_W_ID, O_OL_ID , ID)
    /// simulate the reading and writing of data, and omit the calculation process of data
    ///
    /// insert orderline
    ///
    ...
    /// insert(write) the  OL_NUMBER (column 4) and OL_I_ID (column 5) and OL_SUPPLY_W_ID (column 6) with KEY (OL_W_ID, O_OL_ID , ID)
    /// simulate the IO of data, and omit the calculation process of data
    ///
    /// insert orderline
    ///
    ...
    /// insert(write) the  OL_QUANLITY (column 7) and OL_AMOUNT (column 8) and OL_DIST_INF (column 9) with KEY (OL_W_ID, O_OL_ID , ID)
    /// simulate the IO of data, and omit the calculation process of data
    ///
    /// insert orderline
    ///
    ...
    /// op_0_x -- op_4_x :  Each operation step occupy 1 warp
    /// op_5_x -- op_7_x :  Each operation step occupy 15 warps
    ///
    ...
    /// check raw conflict on warehouse item
    /// Loc is stored in local set
    /// select warehouse (read)
    ...
    /// check raw conflict on district item
    /// select district (read)
    ...
    /// check raw conflict on customer item
    /// select customer (read)
    ...
    /// check war conflict on neworder_set item
    /// insert neworder_set (write)
    ...
    /// check waw conflict on neworder_set item
    /// insert neworder_set (write)
    ...
    /// check war conflict on order item
    /// insert order (write)
    ...
    /// check waw conflict on order item
    /// insert order (write)
    ...
    /// check raw conflict on item data item
    /// each transction check  O_OL_CNT data [5, 15]
    /// select item (read)
    ...
    /// check war conflict on stock item
    /// each transction check  O_OL_CNT data [5, 15] , each warp processes one piece of data
    /// update stock (write)
    ...
    /// check waw conflict on stock item
    /// each transction check  O_OL_CNT data [5, 15] , each warp processes one piece of data
    /// update stock (write)
    ...
    /// check war conflict on orderline item
    /// each transction check  O_OL_CNT data [5, 15] , each warp processes one piece of data
    /// insert orderline (write)
    ...
    /// check waw conflict on orderline item
    /// each transction check  O_OL_CNT data [5, 15] , each warp processes one piece of data
    /// insert orderline (write)
    ...
    /// collect transaction commit information
    ...
    /// write back from local set to snapshot
    /// insert neworder
    ...
    /// update stock  write back.  Each transaction needs to be executed o_ol_cnt times, once per warp.   ( o_ol_cnt [5,15] )
    ...
    /// insert orderline column0-2  write back.  Each transaction needs to be executed o_ol_cnt times, once per warp.   ( o_ol_cnt [5,15] )
    ...
    /// insert orderline column3-5  write back.  Each transaction needs to be executed o_ol_cnt times, once per warp.   ( o_ol_cnt [5,15] )
    ...
    /// insert orderline column6-9  write back.  Each transaction needs to be executed o_ol_cnt times, once per warp.   ( o_ol_cnt [5,15] )
    ...
    /// op_0 , op_3 , op_4  :  Each operation step occupy 1 warp
    /// op_6 -- op_7_x      :  Each operation step occupy 15 warps
    ///
