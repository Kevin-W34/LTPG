...
public:
    ...
    {
        DeliveryQuery query;
        query.NO_W_ID = random.uniform_dist(0, WAREHOUSE_SIZE - 1);
        query.NO_O_ID = random.uniform_dist(0, 29999);

        query.O_ID = random.uniform_dist(0, ORDER_SIZE - 1);
...
public:
    ...
    ORDERSTATUS_SET *orderstatus_set;
    DELIVERY_SET *delivery_set;
    STOCK_SET *stock_set;
    ORDERSTATUS_SET *orderstatus_set_d;
    DELIVERY_SET *delivery_set_d;
    STOCK_SET *stock_set_d;
    ...
    Query()
    {
        ...
        cudaMallocHost((void **)&orderstatus_set, sizeof(ORDERSTATUS_SET));
        cudaMallocHost((void **)&delivery_set, sizeof(DELIVERY_SET));
        cudaMallocHost((void **)&stock_set, sizeof(STOCK_SET));
        ...
        cudaMalloc((void **)&orderstatus_set_d, sizeof(ORDERSTATUS_SET));
        cudaMalloc((void **)&delivery_set_d, sizeof(DELIVERY_SET));
        cudaMalloc((void **)&stock_set_d, sizeof(STOCK_SET));
    ...
    ~Query()
    {
        ...
        cudaFreeHost(orderstatus_set);
        cudaFreeHost(delivery_set);
        cudaFreeHost(stock_set);
        cudaFree(neworder_set_d);
        cudaFree(payment_set_d);
        cudaFree(neworder_query_d);
        cudaFree(payment_query_d);
        cudaFree(orderstatus_set_d);
        cudaFree(delivery_set_d);
        cudaFree(stock_set_d);
        free(makeNeworder);
        free(makePayment);
    }
    void clear_QUERY()
    {
        ...
        cudaMemset(orderstatus_set_d, 0, sizeof(ORDERSTATUS_SET));
        cudaMemset(stock_set_d, 0, sizeof(STOCK_SET));
        cudaMemset(delivery_set_d, 0, sizeof(DELIVERY_SET));
    }
