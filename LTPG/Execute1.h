...
// const hash bucket size, const cooperative groups
...
struct FLAG
{
    ...
    // int R_MIN = 1 << 31 - 1;
    // int W_MIN = 1 << 31 - 1;
    int lock_R[HASH_SIZE]; // 1 unlock, 0 lock
    int TID_LIST_R[HASH_SIZE];
    int lock_W[HASH_SIZE];
    int TID_LIST_W[HASH_SIZE];
};
...
__device__ int hash(int loc, int htblsize, FLAGCOMP *flagcomp)
{
    ...
    //     while (block)
    //     {
    //         if (atomicCAS(&flag[htblloc], (1 << 31 - 1), 0) == 1)
    //         {
    //             int tmp = flag[htblloc];
    //             if (tmp == 1 << 31 - 1 || tmp == loc)
    //             {
    //                 atomicExch(&flag[htblloc].loc, loc);
    //                 atomicExch(&flag[htblloc].lock_loc, 1);
    //                 block = false;
    //             }
    //             else
    //             {
...
namespace NEWORDER_QUERY
{
    ...
    { // select from warehouse
        ...
        int htblloc = Loc; // hash(Loc, OP_SIZE, flag);
        ...
        while (blocked)
        {
            if (atomicCAS(&flag[htblloc].lock_R[TID % HASH_SIZE], 1, 0) == 1)
            {
                // printf("lock %d:0\n", NO);
                atomicMin(&flag[htblloc].TID_LIST_R[TID % HASH_SIZE], TID);
                atomicExch(&flag[htblloc].lock_R[TID % HASH_SIZE], 1);
                blocked = false;
                // printf("unlock %d:0\n", NO);
            }
        }
    ...
    { // insert neworder
        ...
        while (blocked)
        {
            if (atomicCAS(&flag[htblloc].lock_W[TID % HASH_SIZE], 1, 0) == 1)
            {
                // printf("lock %d:3\n", NO);
                atomicMin(&flag[htblloc].TID_LIST_W[TID % HASH_SIZE], TID);
                atomicExch(&flag[htblloc].lock_W[TID % HASH_SIZE], 1);
                blocked = false;
                // printf("unlock %d:3\n", NO);
            }
        }
    ...
    __device__ void execute_n(int *config, GLOBAL *current_2D_d, Set_n *neworder_d, FLAG *flag, FLAGCOMP *flagcomp)
    {
        ...
        else if (wID >= config[20] && wID < config[20] + config[21])
        {
            ...
            while (NO < config[23])
            {
                op_n_5(NO, config, group_split.thread_rank(), current_2D_d, neworder_d, flag, flagcomp);
                op_n_5(NO, config, group_split.thread_rank() + group_split.size() * 1, current_2D_d, neworder_d, flag, flagcomp);
        ...
        else if (wID >= config[24] && wID < config[24] + config[25])
        {
            ...
            while (NO < config[27])
            {
                ...
                //     printf("NO.%d exec\n", NO);
                // }
                op_n_6(NO, config, group_split.thread_rank(), current_2D_d, neworder_d, flag, flagcomp);
                op_n_6(NO, config, group_split.thread_rank() + group_split.size() * 1, current_2D_d, neworder_d, flag, flagcomp);
        ...
        else if (wID >= config[28] && wID < config[28] + config[29])
        {
            ...
            while (NO < config[31])
            {
                ...
                //     printf("NO.%d exec\n", NO);
                // }
                op_n_7(NO, config, group_split.thread_rank(), current_2D_d, neworder_d, flag, flagcomp);
                op_n_7(NO, config, group_split.thread_rank() + group_split.size() * 1, current_2D_d, neworder_d, flag, flagcomp);
    ...
    __device__ void wcheck(int NO, int OID, FLAG *flag, FLAGCOMP *flagcomp, Set_n *neworder_d, int OLID)
    {
        if (OLID < neworder_d[NO].query.O_OL_CNT)
        {
            ...
            int htblloc = Loc; // findhash(Loc, OP_SIZE, flag);
    ...
    __device__ void check_n(int *config, GLOBAL *current_2D_d, Set_n *neworder_d, FLAG *flag, FLAGCOMP *flagcomp)
    {
        ...
        else if (wID >= config[20] && wID < config[20] + config[21])
        {
            ...
            while (NO < config[23])
            {
                ...
                // rcheck(NO, 5 + group_split.thread_rank() + group_split.size() * 2, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 2);
                // rcheck(NO, 5 + group_split.thread_rank() + group_split.size() * 3, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 3);
                // rcheck(NO, 5 + group_split.thread_rank() + group_split.size() * 4, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 4);
                // rcheck(NO, 5 + group_split.thread_rank() + group_split.size() * 5, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 5);
                // rcheck(NO, 5 + group_split.thread_rank() + group_split.size() * 6, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 6);
                // rcheck(NO, 5 + group_split.thread_rank() + group_split.size() * 7, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 7);
                NO += 32 / group_split.size() * config[21];
            }
        }
        else if (wID >= config[24] && wID < config[24] + config[25])
        {
            ...
            while (NO < config[27])
            {
                ...
                // wcheck(NO, 20 + group_split.thread_rank() + group_split.size() * 2, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 2);
                // wcheck(NO, 20 + group_split.thread_rank() + group_split.size() * 3, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 3);
                // wcheck(NO, 20 + group_split.thread_rank() + group_split.size() * 4, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 4);
                // wcheck(NO, 20 + group_split.thread_rank() + group_split.size() * 5, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 5);
                // wcheck(NO, 20 + group_split.thread_rank() + group_split.size() * 6, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 6);
                // wcheck(NO, 20 + group_split.thread_rank() + group_split.size() * 7, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 7);
                NO += 32 / group_split.size() * config[25];
            }
        }
        else if (wID >= config[28] && wID < config[28] + config[29])
        {
            ...
            while (NO < config[31])
            {
                ...
                // wcheck(NO, 35 + group_split.thread_rank() + group_split.size() * 2, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 2);
                // wcheck(NO, 35 + group_split.thread_rank() + group_split.size() * 3, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 3);
                // wcheck(NO, 35 + group_split.thread_rank() + group_split.size() * 4, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 4);
                // wcheck(NO, 35 + group_split.thread_rank() + group_split.size() * 5, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 5);
                // wcheck(NO, 35 + group_split.thread_rank() + group_split.size() * 6, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 6);
                // wcheck(NO, 35 + group_split.thread_rank() + group_split.size() * 7, flag,flag, neworder_d, group_split.thread_rank() + group_split.size() * 7);
                NO += 32 / group_split.size() * config[29];
            }
        }
    }
    ...
    __device__ void write_n(int *config, GLOBAL *current_2D_d, Set_n *neworder_d, FLAG *flag, FLAGCOMP *flagcomp)
    {
        ...
        else if (wID >= config[40] && wID < config[40] + config[41])
        {
            ...
            while (NO < config[43])
            {
                op_n_wb_1(NO, config, group_split.thread_rank(), current_2D_d, neworder_d, flag, flagcomp);
                op_n_wb_1(NO, config, group_split.thread_rank() + group_split.size() * 1, current_2D_d, neworder_d, flag, flagcomp);
                // op_wb_d[1](NO, config, group_split.thread_rank() + group_split.size() * 2, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[1](NO, config, group_split.thread_rank() + group_split.size() * 3, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[1](NO, config, group_split.thread_rank() + group_split.size() * 4, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[1](NO, config, group_split.thread_rank() + group_split.size() * 5, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[1](NO, config, group_split.thread_rank() + group_split.size() * 6, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[1](NO, config, group_split.thread_rank() + group_split.size() * 7, current_2D_d, neworder_d, flag,flag);
        ...
        else if (wID >= config[44] && wID < config[44] + config[45])
        {
            ...
            while (NO < config[47])
            {
                op_n_wb_2(NO, config, group_split.thread_rank(), current_2D_d, neworder_d, flag, flagcomp);
                op_n_wb_2(NO, config, group_split.thread_rank() + group_split.size() * 1, current_2D_d, neworder_d, flag, flagcomp);
                // op_wb_d[2](NO, config, group_split.thread_rank() + group_split.size() * 2, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[2](NO, config, group_split.thread_rank() + group_split.size() * 3, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[2](NO, config, group_split.thread_rank() + group_split.size() * 4, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[2](NO, config, group_split.thread_rank() + group_split.size() * 5, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[2](NO, config, group_split.thread_rank() + group_split.size() * 6, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[2](NO, config, group_split.thread_rank() + group_split.size() * 7, current_2D_d, neworder_d, flag,flag);
        ...
        else if (wID >= config[48] && wID < config[48] + config[49])
        {
            ...
            while (NO < config[51])
            {
                op_n_wb_3(NO, config, group_split.thread_rank(), current_2D_d, neworder_d, flag, flagcomp);
                op_n_wb_3(NO, config, group_split.thread_rank() + group_split.size() * 1, current_2D_d, neworder_d, flag, flagcomp);
                // op_wb_d[3](NO, config, group_split.thread_rank() + group_split.size() * 2, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[3](NO, config, group_split.thread_rank() + group_split.size() * 3, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[3](NO, config, group_split.thread_rank() + group_split.size() * 4, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[3](NO, config, group_split.thread_rank() + group_split.size() * 5, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[3](NO, config, group_split.thread_rank() + group_split.size() * 6, current_2D_d, neworder_d, flag,flag);
                // op_wb_d[3](NO, config, group_split.thread_rank() + group_split.size() * 7, current_2D_d, neworder_d, flag,flag);
    ...
    __device__ void execute_p(int *config, GLOBAL *current_2D_d, Set_p *payment_d, FLAG *flag, FLAGCOMP *flagcomp)
    {
        ...
        if (wID >= config[0] && wID < config[0] + config[1])
        {
            for (size_t i = thID - 32 * config[0]; i < config[3]; i += 32 * config[1])
            {
                ...
                op_p_0(i, config, current_2D_d, payment_d, flag, flagcomp);
            }
        }
        else if (wID >= config[4] && wID < config[4] + config[5])
        {
        ...
        else if (wID >= config[8] && wID < config[8] + config[9])
        {
            for (size_t i = thID - 32 * config[8]; i < config[11]; i += 32 * config[9])
            {
                ...
                op_p_2(i, config, current_2D_d, payment_d, flag, flagcomp);
            }
        }
        else if (wID >= config[12] && wID < config[12] + config[13])
        {
        ...
        else if (wID >= config[16] && wID < config[16] + config[17])
        {
            ...
            {
                // printf("op_p_d[5] wID : %d\t thID : %d\t\n", wID, thID);
                op_p_4(i, config, current_2D_d, payment_d, flag, flagcomp);
