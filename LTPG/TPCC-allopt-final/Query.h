...
public:
    ...
    unsigned int *n_commit;
    unsigned int *p_commit;
    ...
    float copy_to_device_cost = 0.0;
    float copy_to_host_cost = 0.0;
    ...
    Query()
    {
        ...
        // cudaMallocHost((void **)&neworder_set_new, sizeof(NEWORDER_SET) * EPOCH_TP);
        // cudaMallocHost((void **)&payment_set_new, sizeof(PAYMENT_SET) * EPOCH_TP);
        cudaMallocHost((void **)&n_commit, sizeof(unsigned int) * EPOCH_TP);
        cudaMallocHost((void **)&p_commit, sizeof(unsigned int) * EPOCH_TP);
        cudaMalloc((void **)&neworder_set_d, sizeof(NEWORDER_SET) * SLOT_SIZE);
        cudaMalloc((void **)&payment_set_d, sizeof(PAYMENT_SET) * SLOT_SIZE);
        cudaMalloc((void **)&neworder_query_d, sizeof(NEWORDER_QUERY) * SLOT_SIZE);
        cudaMalloc((void **)&payment_query_d, sizeof(PAYMENT_QUERY) * SLOT_SIZE);
        cudaMemset(neworder_set_d, 0, sizeof(NEWORDER_SET) * SLOT_SIZE);
        cudaMemset(payment_set_d, 0, sizeof(PAYMENT_SET) * SLOT_SIZE);
        cudaMemset(neworder_query_d, 0, sizeof(NEWORDER_QUERY) * SLOT_SIZE);
        cudaMemset(payment_query_d, 0, sizeof(PAYMENT_QUERY) * SLOT_SIZE);
    ...
    ~Query()
    {
        ...
        cudaFreeHost(n_commit);
        cudaFreeHost(p_commit);
    ...
    void copy_to_device(unsigned int choose, unsigned int epochID, cudaStream_t *stream)
    {
        ...
        int slotID = epochID % SLOT_SIZE;
        int streamID = epochID;
        // cudaStreamSynchronize(stream[streamID]);
        if (epochID == 0)
        {
            // cudaMemset(&neworder_set_d[0], 0, sizeof(NEWORDER_SET));
            // cudaMemset(&payment_set_d[0], 0, sizeof(PAYMENT_SET));
            // cudaStreamSynchronize(stream[0]);
            // cudaDeviceSynchronize();
            cudaMemcpy(&neworder_query_d[0], &neworder_query[choose], sizeof(NEWORDER_QUERY), cudaMemcpyHostToDevice);
            cudaMemcpy(&payment_query_d[0], &payment_query[choose], sizeof(PAYMENT_QUERY), cudaMemcpyHostToDevice);
        }
        if (slotID == 0)
        {
            // cudaMemset(&neworder_set_d[0], 0, sizeof(NEWORDER_SET));
            // cudaMemset(&payment_set_d[0], 0, sizeof(PAYMENT_SET));
            // cudaMemcpyAsync(&neworder_query_d[i], &neworder_query[choose + i], sizeof(NEWORDER_QUERY), cudaMemcpyHostToDevice, stream[i]);
            // cudaMemcpyAsync(&payment_query_d[i], &payment_query[choose + i], sizeof(PAYMENT_QUERY), cudaMemcpyHostToDevice, stream[i]);
            if (epochID < EPOCH_TP - 1)
            {
                // cudaMemset(&neworder_set_d[0], 0, sizeof(NEWORDER_SET));
                // cudaMemset(&payment_set_d[0], 0, sizeof(PAYMENT_SET));
                // cudaStreamSynchronize(stream[streamID]);
                // cudaDeviceSynchronize();
                cudaMemcpyAsync(&neworder_query_d[1], &neworder_query[choose + 1], sizeof(NEWORDER_QUERY), cudaMemcpyHostToDevice, stream[streamID + 1]);
                cudaMemcpyAsync(&payment_query_d[1], &payment_query[choose + 1], sizeof(PAYMENT_QUERY), cudaMemcpyHostToDevice, stream[streamID + 1]);
            }
        }
        if (slotID == 1)
        {
            // cudaMemset(&neworder_set_d[1], 0, sizeof(NEWORDER_SET));
            // cudaMemset(&payment_set_d[1], 0, sizeof(PAYMENT_SET));
            // cudaMemcpyAsync(&neworder_query_d[i], &neworder_query[choose + i], sizeof(NEWORDER_QUERY), cudaMemcpyHostToDevice, stream[i]);
            // cudaMemcpyAsync(&payment_query_d[i], &payment_query[choose + i], sizeof(PAYMENT_QUERY), cudaMemcpyHostToDevice, stream[i]);
            // cudaMemcpy(&neworder_query_d[1], &neworder_query[choose], sizeof(NEWORDER_QUERY), cudaMemcpyHostToDevice);
            // cudaMemcpy(&payment_query_d[1], &payment_query[choose], sizeof(PAYMENT_QUERY), cudaMemcpyHostToDevice);
            if (epochID < EPOCH_TP - 1)
            {
                // cudaMemset(&neworder_set_d[1], 0, sizeof(NEWORDER_SET));
                // cudaMemset(&payment_set_d[1], 0, sizeof(PAYMENT_SET));
                // cudaStreamSynchronize(stream[streamID]);
                // cudaDeviceSynchronize();
                cudaMemcpyAsync(&neworder_query_d[0], &neworder_query[choose + 1], sizeof(NEWORDER_QUERY), cudaMemcpyHostToDevice, stream[streamID + 1]);
                cudaMemcpyAsync(&payment_query_d[0], &payment_query[choose + 1], sizeof(PAYMENT_QUERY), cudaMemcpyHostToDevice, stream[streamID + 1]);
            }
        }
        // cudaStreamSynchronize(stream[streamID]);
        // cudaDeviceSynchronize();
        // cudaMemset(&neworder_set_d[streamID], 0, sizeof(NEWORDER_SET));
        // cudaMemset(&payment_set_d[streamID], 0, sizeof(PAYMENT_SET));
        // cudaMemcpyAsync(&neworder_query_d[streamID], &neworder_query[choose], sizeof(NEWORDER_QUERY), cudaMemcpyHostToDevice, stream);
        // cudaMemcpyAsync(&payment_query_d[streamID], &payment_query[choose], sizeof(PAYMENT_QUERY), cudaMemcpyHostToDevice, stream);
    ...
    void copy_to_host(unsigned int epochID, cudaStream_t *stream)
    {
        ...
        int slotID = epochID % SLOT_SIZE;
        // cudaStreamSynchronize(stream);
        // cudaMemcpyAsync(&neworder_set_new[epochID].COMMIT, &neworder_set_d[streamID].COMMIT, sizeof(unsigned int), cudaMemcpyDeviceToHost, stream);
        // cudaMemcpyAsync(&payment_set_new[epochID].COMMIT, &payment_set_d[streamID].COMMIT, sizeof(unsigned int), cudaMemcpyDeviceToHost, stream);
        // cudaStreamSynchronize(stream);
        // cudaMemcpy(&neworder_set_new[epochID].COMMIT, &neworder_set_d[streamID].COMMIT, sizeof(unsigned int), cudaMemcpyDeviceToHost);
        // cudaMemcpy(&payment_set_new[epochID].COMMIT, &payment_set_d[streamID].COMMIT, sizeof(unsigned int), cudaMemcpyDeviceToHost);
        // cudaStreamSynchronize(stream[streamID]);

        cudaMemcpyAsync(&n_commit[epochID], &neworder_set_d[slotID].COMMIT, sizeof(unsigned int), cudaMemcpyDeviceToHost,stream[epochID]);
        cudaMemcpyAsync(&p_commit[epochID], &payment_set_d[slotID].COMMIT, sizeof(unsigned int), cudaMemcpyDeviceToHost,stream[epochID]);
        cudaStreamSynchronize(stream[epochID]);
        cudaMemset(&neworder_set_d[slotID], 0, sizeof(NEWORDER_SET));
        cudaMemset(&payment_set_d[slotID], 0, sizeof(PAYMENT_SET));

        long long end = current_time();
        float time = duration(start, end);
        if (epochID >= WARMUP_TP && epochID < EPOCH_TP - WARMUP_TP)
        {
            this->copy_to_host_cost += time;
        }
...
{ // analyse executed queries
    ...
    if (epochID >= WARMUP_TP && epochID < EPOCH_TP - WARMUP_TP)
    {
        query->commit_neworder += query->n_commit[epochID];
        query->commit_payment += query->p_commit[epochID];
        // std::cout << "commit neworder " << query->n_commit[epochID] << std::endl;
        // std::cout << "commit payment " << query->p_commit[epochID] << std::endl;
