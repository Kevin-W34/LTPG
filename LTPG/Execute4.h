...
// change hash table size, change cooperative groups
...
namespace NEWORDER_QUERY
{
    ...
    __device__ void execute_n(int *config, GLOBAL *current_2D_d, Set_n *neworder_d, FLAG *flag, FLAGCOMP *flagcomp)
    {
        ...
        {
            for (int i = thID - BLOCKSIZE * config[0]; i < config[3]; i += BLOCKSIZE * config[1])
            {
                // printf("op_n_d[0] wID : %d\t thID : %d\t\n", wID, thID);
                op_n_0(i, config, 0, current_2D_d, neworder_d, flag, flagcomp);
            }
        }
        else if (wID >= config[4] && wID < config[4] + config[5])
        {
            for (int i = thID - BLOCKSIZE * config[4]; i < config[7]; i += BLOCKSIZE * config[5])
            {
                // printf("op_n_d[1] wID : %d\t thID : %d\t\n", wID, thID);
                op_n_1(i, config, 0, current_2D_d, neworder_d, flag, flagcomp);
            }
        }
        else if (wID >= config[8] && wID < config[8] + config[9])
        {
            for (int i = thID - BLOCKSIZE * config[8]; i < config[11]; i += BLOCKSIZE * config[9])
            {
                // printf("op_n_d[2] wID : %d\t thID : %d\t\n", wID, thID);
                op_n_2(i, config, 0, current_2D_d, neworder_d, flag, flagcomp);
            }
        }
        else if (wID >= config[12] && wID < config[12] + config[13])
        {
            for (int i = thID - BLOCKSIZE * config[12]; i < config[15]; i += BLOCKSIZE * config[13])
            {
                // printf("op_n_d[3] wID : %d\t thID : %d\t\n", wID, thID);
                op_n_3(i, config, 0, current_2D_d, neworder_d, flag, flagcomp);
            }
        }
        else if (wID >= config[16] && wID < config[16] + config[17])
        {
            for (int i = thID - BLOCKSIZE * config[16]; i < config[19]; i += BLOCKSIZE * config[17])
            {
                // printf("op_n_d[5] wID : %d\t thID : %d\t\n", wID, thID);
                // op_d[4](i, config, 0, current_2D_d, neworder_d, flag);
                op_n_4(i, config, 0, current_2D_d, neworder_d, flag, flagcomp);
            }
        }
        else if (wID >= config[20] && wID < config[20] + config[21])
        {
            thread_block group = this_thread_block();
            // thread_block_tile<32> group32 = tiled_partition<32>(group);
            // __shared__ int size_of_thd_in_cg[16];
            int cg_size = config[22];
            thread_block_tile<CG_SIZE> group_split = tiled_partition<CG_SIZE>(group);
            int group_id = group.thread_rank() % cg_size;
            int NO = (thID - BLOCKSIZE * config[20]) / cg_size;

            while (NO < config[23])
            {
                int size = neworder_d[NO].query.O_OL_CNT;
                // if (group.thread_rank() == 0)
                // {
                //     for (size_t ii = 0; ii < cg_size; ii++)
                //     {
                //         size_of_thd_in_cg[ii] = group.shfl(size, 0);
                //     }
                // }
                for (int i = 0; size > 0; i++)
                {
                    op_n_5(NO, config, group_id + cg_size * i, current_2D_d, neworder_d, flag, flagcomp);
                    size -= cg_size;
                }
                // op_d[5](NO, config, group_split.thread_rank() + group_split.size() * 0, current_2D_d, neworder_d, flag, flagcomp);
                ...
                NO += BLOCKSIZE / cg_size * config[21];
            }
        }
        else if (wID >= config[24] && wID < config[24] + config[25])
        {
            thread_block group = this_thread_block();
            // thread_block_tile<32> group32 = tiled_partition<32>(group);
            int cg_size = config[26];
            thread_block_tile<CG_SIZE> group_split = tiled_partition<CG_SIZE>(group);
            int group_id = group.thread_rank() % cg_size;
            int NO = (thID - BLOCKSIZE * config[24]) / cg_size;
            while (NO < config[27])
            {
                int size = neworder_d[NO].query.O_OL_CNT;
                for (int i = 0; size > 0; i++)
                {
                    op_n_6(NO, config, group_id + cg_size * i, current_2D_d, neworder_d, flag, flagcomp);
                    size -= cg_size;
                }
                // op_d[6](NO, config, group_split.thread_rank() + group_split.size() * 0, current_2D_d, neworder_d, flag, flagcomp);
                ...
                NO += BLOCKSIZE / cg_size * config[25];
            }
        }
        else if (wID >= config[28] && wID < config[28] + config[29])
        {
            thread_block group = this_thread_block();
            // thread_block_tile<32> group32 = tiled_partition<32>(group);
            int cg_size = config[30];
            thread_block_tile<CG_SIZE> group_split = tiled_partition<CG_SIZE>(group);
            int group_id = group.thread_rank() % cg_size;
            int NO = (thID - BLOCKSIZE * config[28]) / cg_size;
            while (NO < config[31])
            {
                int size = neworder_d[NO].query.O_OL_CNT;
                for (int i = 0; size > 0; i++)
                {
                    op_n_7(NO, config, group_id + cg_size * i, current_2D_d, neworder_d, flag, flagcomp);
                    size -= cg_size;
                }
                // op_d[7](NO, config, group_split.thread_rank() + group_split.size() * 0, current_2D_d, neworder_d, flag, flagcomp);
                ...
                NO += BLOCKSIZE / cg_size * config[29];
            }
        }
    }
    ...
    __device__ void wcheck(int NO, int OID, FLAG *flag, FLAGCOMP *flagcomp, Set_n *neworder_d, int OLID)
    {
        if (OLID < neworder_d[NO].query.O_OL_CNT)
        {
            ...
            if (flag[htblloc].W_CNT > 1)
            {
                ...
                {
                    for (int i = 0; i < COMPETITION_HASH_SIZE; i++)
                    {
                ...
                {
                    for (int i = 0; i < NORMAL_HASH_SIZE; i++)
                    {
    ...
    __device__ void check_n(int *config, GLOBAL *current_2D_d, Set_n *neworder_d, FLAG *flag, FLAGCOMP *flagcomp)
    {
        ...
        if (wID >= config[0] && wID < config[0] + config[1])
        {
            for (int i = thID - BLOCKSIZE * config[0]; i < config[2] + config[3]; i += BLOCKSIZE * config[1])
            {
        ...
        {
            for (int i = thID - BLOCKSIZE * config[4]; i < config[6] + config[7]; i += BLOCKSIZE * config[5])
            {
        ...
        {
            for (int i = thID - BLOCKSIZE * config[8]; i < config[10] + config[11]; i += BLOCKSIZE * config[9])
            {
        ...
        {
            for (int i = thID - BLOCKSIZE * config[12]; i < config[14] + config[15]; i += BLOCKSIZE * config[13])
            {
        ...
        {
            for (int i = thID - BLOCKSIZE * config[16]; i < config[18] + config[19]; i += BLOCKSIZE * config[17])
            {
        ...
        else if (wID >= config[20] && wID < config[20] + config[21])
        {
            ...
            while (NO < config[23])
            {
                ...
                for (int i = 0; size > 0; i++)
                {
                    rcheck(NO, 5 + group_id + cg_size * i, flag, flagcomp, neworder_d, group_id + cg_size * i);
                    size -= cg_size;
                }
        ...
        else if (wID >= config[24] && wID < config[24] + config[25])
        {
            ...
            while (NO < config[27])
            {
                ...
                for (int i = 0; size > 0; i++)
                {
                    wcheck(NO, 20 + group_id + cg_size * i, flag, flagcomp, neworder_d, group_id + cg_size * i);
                    size -= cg_size;
                }
        ...
        else if (wID >= config[28] && wID < config[28] + config[29])
        {
            ...
            while (NO < config[31])
            {
                ...
                for (int i = 0; size > 0; i++)
                {
                    wcheck(NO, 35 + group_id + cg_size * i, flag, flagcomp, neworder_d, group_id + cg_size * i);
                    size -= cg_size;
                }
    ...
    __device__ void write_n(int *config, GLOBAL *current_2D_d, Set_n *neworder_d, FLAG *flag, FLAGCOMP *flagcomp)
    {
        int thID = threadIdx.x + blockDim.x * blockIdx.x;
        int wID = thID / BLOCKSIZE;
        if (wID >= config[32] && wID < config[32] + config[33])
        {
            for (int i = thID - BLOCKSIZE * config[32]; i < config[35]; i += BLOCKSIZE * config[33])
            {
                // printf("op_wb_d[0] wID : %d\t thID : %d\t\n", wID, thID);

                op_n_wb_0(i, config, 0, current_2D_d, neworder_d, flag, flagcomp);
            }
        }
        else if (wID >= config[36] && wID < config[36] + config[37])
        {
            thread_block group = this_thread_block();
            // thread_block_tile<32> group32 = tiled_partition<32>(group);
            int cg_size = config[38];
            thread_block_tile<CG_SIZE> group_split = tiled_partition<CG_SIZE>(group);
            int group_id = group.thread_rank() % cg_size;
            int NO = (thID - BLOCKSIZE * config[40]) / cg_size;
            while (NO < config[39])
            {
                int size = neworder_d[NO].query.O_OL_CNT;
                for (int i = 0; size > 0; i++)
                {
                    op_n_wb_1(NO, config, group_id + cg_size * i, current_2D_d, neworder_d, flag, flagcomp);
                    size -= cg_size;
                }
                // op_wb_d[1](NO, config, group_split.thread_rank() + group_split.size() * 0, current_2D_d, neworder_d, flag, flagcomp);
                ...
                NO += BLOCKSIZE / cg_size * config[37];
            }
        }
        else if (wID >= config[40] && wID < config[40] + config[41])
        {
            thread_block group = this_thread_block();
            // thread_block_tile<32> group32 = tiled_partition<32>(group);
            int cg_size = config[42];
            thread_block_tile<CG_SIZE> group_split = tiled_partition<CG_SIZE>(group);
            int group_id = group.thread_rank() % cg_size;
            int NO = (thID - BLOCKSIZE * config[44]) / cg_size;
            while (NO < config[43])
            {
                int size = neworder_d[NO].query.O_OL_CNT;
                for (int i = 0; size > 0; i++)
                {
                    op_n_wb_2(NO, config, group_id + cg_size * i, current_2D_d, neworder_d, flag, flagcomp);
                    size -= cg_size;
                }
                // op_wb_d[2](NO, config, group_split.thread_rank() + group_split.size() * 0, current_2D_d, neworder_d, flag, flagcomp);
                ...
                NO += BLOCKSIZE / cg_size * config[41];
            }
        }
        else if (wID >= config[44] && wID < config[44] + config[45])
        {
            thread_block group = this_thread_block();
            // thread_block_tile<32> group32 = tiled_partition<32>(group);
            int cg_size = config[46];
            thread_block_tile<CG_SIZE> group_split = tiled_partition<CG_SIZE>(group);
            int group_id = group.thread_rank() % cg_size;
            int NO = (thID - BLOCKSIZE * config[48]) / cg_size;
            while (NO < config[47])
            {
                int size = neworder_d[NO].query.O_OL_CNT;
                for (int i = 0; size > 0; i++)
                {
                    op_n_wb_3(NO, config, group_id + cg_size * i, current_2D_d, neworder_d, flag, flagcomp);
                    size -= cg_size;
                }
                // op_wb_d[3](NO, config, group_split.thread_rank() + group_split.size() * 0, current_2D_d, neworder_d, flag, flagcomp);
                ...
                NO += BLOCKSIZE / cg_size * config[45];
            }
        }
    }
};
...
namespace PAYMENT_QUERY
{
    ...
    __device__ void execute_p(int *config,
                              GLOBAL *current_2D_d,
                              Set_p *payment_d,
                              FLAG *flag,
                              FLAGCOMP *flagcomp,
                              ...
                              int *c_Loc_d)
    {
        ...
        if (wID >= config[0] && wID < config[0] + config[1])
        {
            for (int i = thID - BLOCKSIZE * config[0]; i < config[3]; i += BLOCKSIZE * config[1])
            {
                ...
                op_p_0(i, config, current_2D_d, payment_d, flag, flagcomp, w_ID_d, w_Loc_d);
            }
        }
        else if (wID >= config[4] && wID < config[4] + config[5])
        {
            for (int i = thID - BLOCKSIZE * config[4]; i < config[7]; i += BLOCKSIZE * config[5])
            {
                ...
                op_p_1(i, config, current_2D_d, payment_d, flag, flagcomp);
            }
        }
        else if (wID >= config[8] && wID < config[8] + config[9])
        {
            for (int i = thID - BLOCKSIZE * config[8]; i < config[11]; i += BLOCKSIZE * config[9])
            {
                ...
                op_p_2(i, config, current_2D_d, payment_d, flag, flagcomp, d_ID_d, d_Loc_d);
            }
        }
        else if (wID >= config[12] && wID < config[12] + config[13])
        {
            for (int i = thID - BLOCKSIZE * config[12]; i < config[15]; i += BLOCKSIZE * config[13])
            {
                ...
                op_p_3(i, config, current_2D_d, payment_d, flag, flagcomp);
            }
        }
        else if (wID >= config[16] && wID < config[16] + config[17])
        {
            ...
            {
                // printf("op_p_d[5] wID : %d\t thID : %d\t\n", wID, thID);
                op_p_4(i, config, current_2D_d, payment_d, flag, flagcomp, c_ID_d, c_Loc_d);
                op_p_5(i, config, current_2D_d, payment_d, flag, flagcomp);
            }
        }
        else if (wID >= config[20] && wID < config[20] + config[21])
        {
            for (int i = thID - BLOCKSIZE * config[20]; i < config[23]; i += BLOCKSIZE * config[21])
            {
                // printf("op_p_d[6] wID : %d\t thID : %d\t\n", wID, thID);

                op_p_6(i, config, current_2D_d, payment_d, flag, flagcomp);
            }
        }
    }

    __device__ void wcheck(FLAG *flag,
                           FLAGCOMP *flagcomp,
                           Set_p *payment_d,
                           int NO,
                           int OID)
    ...
    __device__ void rcheck(FLAG *flag,
                           FLAGCOMP *flagcomp,
                           Set_p *payment_d,
                           int NO,
                           int OID)
    ...
    __device__ void check_p(int *config,
                            GLOBAL *current_2D_d,
                            Set_p *payment_d,
                            FLAG *flag,
                            FLAGCOMP *flagcomp)
    ...
    __device__ void op_p_wb_0(int NO,
                              int *config,
                              GLOBAL *current_2D_d,
                              Set_p *payment_d,
                              FLAG *flag,
    ...
    __device__ void op_p_wb_1(int NO,
                              int *config,
                              GLOBAL *current_2D_d,
                              Set_p *payment_d,
                              FLAG *flag,
    ...
    __device__ void op_p_wb_2(int NO,
                              int *config,
                              GLOBAL *current_2D_d,
                              Set_p *payment_d,
                              FLAG *flag,
    ...
    __device__ void op_p_wb_3(int NO,
                              int *config,
                              GLOBAL *current_2D_d,
                              Set_p *payment_d,
                              FLAG *flag,
    ...
    __device__ void write_p(int *config,
                            GLOBAL *current_2D_d,
                            Set_p *payment_d,
                            FLAG *flag,
                            FLAGCOMP *flagcomp)
    {
        ...
        {
            for (int i = thID - BLOCKSIZE * config[24]; i < config[27]; i += BLOCKSIZE * config[25])
            {
                // printf("op_wb_d[0] wID : %d\t thID : %d\t\n", wID, thID);
                op_p_wb_0(i, config, current_2D_d, payment_d, flag, flagcomp);
            }
        }
        else if (wID >= config[28] && wID < config[28] + config[29])
        {
            for (int i = thID - BLOCKSIZE * config[28]; i < config[31]; i += BLOCKSIZE * config[29])
            {
                // printf("op_wb_d[1] wID : %d\t thID : %d\t\n", wID, thID);
                op_p_wb_1(i, config, current_2D_d, payment_d, flag, flagcomp);
            }
        }
        else if (wID >= config[32] && wID < config[32] + config[33])
        {
            ...
            {
                // printf("op_wb_d[2] wID : %d\t thID : %d\t\n", wID, thID);
                op_p_wb_2(i, config, current_2D_d, payment_d, flag, flagcomp);
            }
        }
        else if (wID >= config[36] && wID < config[36] + config[37])
        {
            for (int i = thID - BLOCKSIZE * config[36]; i < config[39]; i += BLOCKSIZE * config[37])
            {
                // printf("op_wb_d[3] wID : %d\t thID : %d\t\n", wID, thID);
                op_p_wb_3(i, config, current_2D_d, payment_d, flag, flagcomp);
            }
        }
    }

    __device__ void hco_0(int NO,
                          int *config,
                          GLOBAL *current_2D_d,
                          Set_p *payment_d,
                          int *w_ID_d,
    ...
    { // update warehouse
        ...
        int pre_state = payment_d[pre_ID].state;
        GLOBAL tmp = payment_d[ID].set[0].local_set;
        bool blocked = true;
        while (blocked)
        {
            if (pre_state == 0)
            {
                current_2D_d[Loc].data[2] += tmp.data[2];
                payment_d[ID].set[1].local_set.data[3] = current_2D_d[Loc].data[3];
                payment_d[ID].set[1].local_set.data[4] = current_2D_d[Loc].data[4];
                payment_d[ID].set[1].local_set.data[5] = current_2D_d[Loc].data[5];
                payment_d[ID].set[1].local_set.data[6] = current_2D_d[Loc].data[6];
                payment_d[ID].set[1].local_set.data[7] = current_2D_d[Loc].data[7];
                payment_d[ID].set[1].local_set.data[8] = current_2D_d[Loc].data[8];
                atomicExch(&payment_d[ID].state, 0);
                blocked = false;
            }
        }
    }

    __device__ void hco_2(int NO,
                          int *config,
                          GLOBAL *current_2D_d,
                          Set_p *payment_d,
                          int *d_ID_d,
    ...
    { // update district
        ...
        while (blocked)
        {
            if (pre_state == 0)
            {
                current_2D_d[Loc].data[3] += tmp.data[3];
                payment_d[ID].set[3].local_set.data[5] = current_2D_d[Loc].data[5];
                payment_d[ID].set[3].local_set.data[6] = current_2D_d[Loc].data[6];
                payment_d[ID].set[3].local_set.data[7] = current_2D_d[Loc].data[7];
                payment_d[ID].set[3].local_set.data[8] = current_2D_d[Loc].data[8];
                payment_d[ID].set[3].local_set.data[9] = current_2D_d[Loc].data[9];
                payment_d[ID].set[3].local_set.data[10] = current_2D_d[Loc].data[10];
    ...
    { // update customer
        ...
        while (blocked)
        {
            if (pre_state == 0)
            {
                int C_CREDIT = current_2D_d[Loc].data[10];
                if (C_CREDIT)
                {
                    current_2D_d[Loc].data[5] += tmp.data[5];
                }
                else
                {
                    current_2D_d[Loc].data[9] += tmp.data[9];
                }
                payment_d[ID].set[4].local_set.data[3] = current_2D_d[Loc].data[3];
                payment_d[ID].set[4].local_set.data[4] = current_2D_d[Loc].data[4];
                payment_d[ID].set[4].local_set.data[5] = current_2D_d[Loc].data[5];
                payment_d[ID].set[4].local_set.data[11] = current_2D_d[Loc].data[11];
                payment_d[ID].set[4].local_set.data[12] = current_2D_d[Loc].data[12];
                payment_d[ID].set[4].local_set.data[13] = current_2D_d[Loc].data[13];
                payment_d[ID].set[4].local_set.data[14] = current_2D_d[Loc].data[14];
                payment_d[ID].set[4].local_set.data[15] = current_2D_d[Loc].data[15];
                payment_d[ID].set[4].local_set.data[16] = current_2D_d[Loc].data[16];
                payment_d[ID].set[4].local_set.data[17] = current_2D_d[Loc].data[17];
                payment_d[ID].set[4].local_set.data[18] = current_2D_d[Loc].data[18];
                payment_d[ID].set[4].local_set.data[19] = current_2D_d[Loc].data[19];
                payment_d[ID].set[4].local_set.data[20] = current_2D_d[Loc].data[20];
                atomicExch(&payment_d[ID].state, 0);
                blocked = false;
            }
        }
    ...
    __device__ void exec_hco(int *config,
                             GLOBAL *current_2D_d,
                             Set_p *payment_d,
                             int count,
                             int *w_ID_d,
                             ...
                             int *c_Loc_d)
    {
        ...
        while (thID < count)
        {
            hco_0(thID, config, current_2D_d, payment_d, w_ID_d, w_Loc_d);
            thID += 68 * 1024;
        }
        thID = threadIdx.x + blockDim.x * blockIdx.x;
        while (thID < count)
        {
            hco_2(thID, config, current_2D_d, payment_d, d_ID_d, d_Loc_d);
            thID += 68 * 1024;
        }
        thID = threadIdx.x + blockDim.x * blockIdx.x;
        while (thID < count)
        {
            hco_5(thID, config, current_2D_d, payment_d, c_ID_d, c_Loc_d);
            thID += 68 * 1024;
        }
    }
}