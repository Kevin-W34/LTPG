...
namespace YCSB_A_NAMESPACE
{
                           ...
                           YCSB_INDEX *index)
    {
        ...
        // printf("n op_0_0 %d\n", TID);
        for (size_t i = 0; i < YCSB_SCAN_SIZE; i++)
        {
            if (Loc + i < YCSB_SIZE)
            {
                unsigned int log_loc_up = (Loc + i >> 8);
                unsigned int log_loc_down = (Loc + i << 24) + READ_TYPE;
                YCSB::__REGISTER(0, NO, OP, YCSB_A_SIZE, TID, Loc + i, log_loc_up, log_loc_down, log);
            }
        }
    }
                            ...
                            YCSB_INDEX *index)
    {
        ...
        for (size_t i = 0; i < YCSB_SCAN_SIZE; i++)
        {
            if (Loc + i < YCSB_SIZE)
            {
                unsigned int log_loc_up = (Loc + i >> 8);
                unsigned int log_loc_down = (Loc + i << 24) + READ_TYPE;
                YCSB::__CHECK(0, NO, Loc + 1, TID, log_loc_up, log_loc_down, ycsb_set->raw, log);
            }
        }
    }
