...
namespace ORDERSTATUSQUERY_NAMESPACE
{
    ...
    { // select customer
        ...
        unsigned int DID = __ldg(&query->query[NO].D_ID);
        unsigned int CID = __ldg(&query->query[NO].C_ID);
        unsigned int TID = __ldg(&query->TID[NO]);
        unsigned int Loc = WID * 30000 + DID * 3000 + CID;
        unsigned int log_loc_up = (CUSTOMER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + READ_TYPE;
        query->set_Loc[0 * ORDERSTATUS_CNT + NO] = Loc;
    ...
    { // select order
        ...
        unsigned int TID = __ldg(&query->TID[NO]);
        unsigned int Loc = WID * 30000 + OID;
        query->set_Loc[1 * ORDERSTATUS_CNT + NO] = Loc;
        unsigned int log_loc_up = (ORDER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + READ_TYPE;
    ...
    { // select orderline
        ...
        query->set_Loc[2 * ORDERSTATUS_CNT + NO] = Loc;
        unsigned int log_loc_up = (ORDERLINE_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + READ_TYPE;
                          ...
                          INDEX *index)
    {
        ...
        if (NO < ORDERSTATUS_CNT)
        {
            ...
            {
                check_1(NO, query, log, snapshot, index);
            }
            else if (wID % CHECK_WARP == 2)
            {
                check_2(NO, query, log, snapshot, index);
            }
        }
    }
...
namespace DELIVERYQUERY_NAMESPACE
{
    __device__ void op_0_0(unsigned int NO,
                           ...
                           INDEX *index)
    { // delete neworder
        unsigned int NOID = query->query[NO].NO_O_ID;
        unsigned int NWID = query->query[NO].NO_W_ID;
        unsigned int TID = query->TID[NO];
        unsigned int Loc = NWID * 30000 + NOID;
        unsigned int log_loc_up = (NEWORDER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + WRITE_TYPE;
        query->set_Loc[0 * DELIVERY_CNT + NO] = Loc;
        __REGISTER(0, NO, 0, DELIVERY_CNT, TID, Loc, log_loc_up, log_loc_down, log);
    }
    __device__ void op_0_1(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
    ...
    { // delete neworder
        ...
        unsigned int NWID = query->query[NO].NO_W_ID;
        unsigned int Loc = NWID * 30000 + NOID;
        __WRITE(NO, 0, 0, Loc, DELIVERY_CNT, NEWORDER_SIZE, NEWORDER_COLUMN, 0, query->set_local_set, snapshot->neworder_snapshot);
    }
    __device__ void op_1_0(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
                           ...
                           INDEX *index)
    { // select order
        unsigned int OID = query->query[NO].O_ID;
        unsigned int TID = query->TID[NO];
        unsigned int Loc = OID;
        unsigned int log_loc_up = (ORDER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + READ_TYPE;
        query->set_Loc[1 * DELIVERY_CNT + NO] = Loc;
        __REGISTER(0, NO, 1, DELIVERY_CNT, TID, Loc, log_loc_up, log_loc_down, log);
    }
    __device__ void op_1_1(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
    ...
    { // select order
        unsigned int OID = query->query[NO].O_ID;
        unsigned int Loc = OID;
        __READ(NO, 1, 3, Loc, DELIVERY_CNT, ORDER_SIZE, ORDER_COLUMN, 0, query->set_local_set, snapshot->order_snapshot);
    }
    __device__ void op_2_0(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
    ...
    { // update order
        ...
        unsigned int log_loc_up = (ORDER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + WRITE_TYPE;
        query->set_Loc[2 * DELIVERY_CNT + NO] = Loc;
        __REGISTER(0, NO, 2, DELIVERY_CNT, TID, Loc, log_loc_up, log_loc_down, log);
    }
    __device__ void op_2_1(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
    ...
    { // update order
        unsigned int OID = query->query[NO].O_ID;
        unsigned int Loc = OID;
        __WRITE(NO, 2, 3, Loc, DELIVERY_CNT, ORDER_SIZE, ORDER_COLUMN, 0, query->set_local_set, snapshot->order_snapshot);
    }
    __device__ void op_3_0(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
    ...
    { // update orderline
        ...
        unsigned int log_loc_up = (ORDERLINE_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + WRITE_TYPE;
        query->set_Loc[3 * DELIVERY_CNT + NO] = Loc;
        __REGISTER(0, NO, 3, DELIVERY_CNT, TID, Loc, log_loc_up, log_loc_down, log);
    }
    __device__ void op_3_1(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
    ...
    { // update orderline
        unsigned int OID = query->query[NO].O_ID;
        unsigned int Loc = OID;
        __WRITE(NO, 3, 3, Loc, DELIVERY_CNT, ORDERLINE_SIZE, ORDERLINE_COLUMN, 0, query->set_local_set, snapshot->orderline_snapshot);
    }
    __device__ void op_4_0(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
    ...
    { // select orderline
        ...
        unsigned int log_loc_up = (ORDERLINE_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + READ_TYPE;
        query->set_Loc[4 * DELIVERY_CNT + NO] = Loc;
        __REGISTER(0, NO, 4, DELIVERY_CNT, TID, Loc, log_loc_up, log_loc_down, log);
    }
    __device__ void op_4_1(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
    ...
    { // selct orderline
        unsigned int OID = query->query[NO].O_ID;
        unsigned int Loc = OID;
        __READ(NO, 4, 3, Loc, DELIVERY_CNT, ORDERLINE_SIZE, ORDERLINE_COLUMN, 0, query->set_local_set, snapshot->orderline_snapshot);
    }
    __device__ void op_5_0(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
    ...
    { // update customer
        ...
        unsigned int log_loc_up = (CUSTOMER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + WRITE_TYPE;
        query->set_Loc[5 * DELIVERY_CNT + NO] = Loc;
        __REGISTER(0, NO, 5, DELIVERY_CNT, TID, Loc, log_loc_up, log_loc_down, log);
    }
    __device__ void op_5_1(unsigned int NO,
                           DELIVERY_SET *query,
                           LOG *log,
    ...
    { // update customer
        unsigned int OID = query->query[NO].O_ID;
        unsigned int Loc = OID;
        __WRITE(NO, 5, 3, Loc, DELIVERY_CNT, CUSTOMER_SIZE, CUSTOMER_COLUMN, 0, query->set_local_set, snapshot->customer_snapshot);
    }
    __device__ void execute(unsigned int NO,
                            DELIVERY_SET *query,
                            LOG *log,
                            ...
                            INDEX *index)
    {
        ...
        if (NO < DELIVERY_CNT)
        {
            ...
            {
                op_3_0(NO, query, log, snapshot, index);
            }
            else if (wID % EXECUTE_WARP == 7)
            {
                op_3_1(NO, query, log, snapshot, index);
            }
            else if (wID % EXECUTE_WARP == 8)
            {
                op_4_0(NO, query, log, snapshot, index);
            }
            else if (wID % EXECUTE_WARP == 9)
            {
                op_4_1(NO, query, log, snapshot, index);
            }
            else if (wID % EXECUTE_WARP == 10)
            {
                op_5_0(NO, query, log, snapshot, index);
            }
            else if (wID % EXECUTE_WARP == 11)
            {
                op_5_1(NO, query, log, snapshot, index);
            }
        }
    }
    __device__ void check_0_0(unsigned int NO,
    ...
    {
        unsigned int TID = query->TID[NO];
        unsigned int Loc = query->set_Loc[0 * DELIVERY_CNT + NO];
        unsigned int log_loc_up = (NEWORDER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + READ_TYPE;
        __CHECK(0, NO, Loc, TID, log_loc_up, log_loc_down, query->raw, log);
    }
    __device__ void check_0_1(unsigned int NO,
                              DELIVERY_SET *query,
                              LOG *log,
                              SNAPSHOT *snapshot,
                              INDEX *index)
    ...
    {
        unsigned int TID = query->TID[NO];
        unsigned int Loc = query->set_Loc[1 * DELIVERY_CNT + NO];
        unsigned int log_loc_up = (ORDER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + WRITE_TYPE;
        __CHECK(1, NO, Loc, TID, log_loc_up, log_loc_down, query->war, log);
    }
    __device__ void check_2_0(unsigned int NO,
                              DELIVERY_SET *query,
                              LOG *log,
    ...
    {
        unsigned int TID = query->TID[NO];
        unsigned int Loc = query->set_Loc[2 * DELIVERY_CNT + NO];
        unsigned int log_loc_up = (ORDER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + READ_TYPE;
        __CHECK(2, NO, Loc, TID, log_loc_up, log_loc_down, query->raw, log);
    }
    __device__ void check_2_1(unsigned int NO,
                              DELIVERY_SET *query,
                              LOG *log,
                              SNAPSHOT *snapshot,
                              INDEX *index)
    {
        ...
        unsigned int log_loc_up = (ORDER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + WRITE_TYPE;
        __CHECK(2, NO, Loc, TID, log_loc_up, log_loc_down, query->waw, log);
    }
    __device__ void check_3_0(unsigned int NO,
                              DELIVERY_SET *query,
                              LOG *log,
    ...
    {
        unsigned int TID = query->TID[NO];
        unsigned int Loc = query->set_Loc[3 * DELIVERY_CNT + NO];
        unsigned int log_loc_up = (ORDERLINE_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + READ_TYPE;
        __CHECK(3, NO, Loc, TID, log_loc_up, log_loc_down, query->raw, log);
    }
    __device__ void check_3_1(unsigned int NO,
                              DELIVERY_SET *query,
                              LOG *log,
                              ...
                              INDEX *index)
    {
        ...
        unsigned int log_loc_up = (ORDERLINE_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + WRITE_TYPE;
        __CHECK(3, NO, Loc, TID, log_loc_up, log_loc_down, query->waw, log);
    }
    __device__ void check_4_0(unsigned int NO,
                              DELIVERY_SET *query,
                              LOG *log,
    ...
    {
        unsigned int TID = query->TID[NO];
        unsigned int Loc = query->set_Loc[4 * DELIVERY_CNT + NO];
        unsigned int log_loc_up = (ORDERLINE_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + WRITE_TYPE;
        __CHECK(4, NO, Loc, TID, log_loc_up, log_loc_down, query->war, log);
    }
    __device__ void check_5_0(unsigned int NO,
                              DELIVERY_SET *query,
                              LOG *log,
    ...
    {
        unsigned int TID = query->TID[NO];
        unsigned int Loc = query->set_Loc[5 * DELIVERY_CNT + NO];
        unsigned int log_loc_up = (CUSTOMER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + READ_TYPE;
        __CHECK(5, NO, Loc, TID, log_loc_up, log_loc_down, query->raw, log);
    }
    __device__ void check_5_1(unsigned int NO,
                              DELIVERY_SET *query,
                              LOG *log,
                              ...
                              INDEX *index)
    {
        ...
        unsigned int log_loc_up = (CUSTOMER_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + WRITE_TYPE;
        __CHECK(5, NO, Loc, TID, log_loc_up, log_loc_down, query->waw, log);
    }
    __device__ void check(unsigned int NO,
                          DELIVERY_SET *query,
                          LOG *log,
                          ...
                          INDEX *index)
    {
        ...
        if (NO < DELIVERY_CNT)
        {
            if (wID % CHECK_WARP == 0)
            {
                check_0_0(NO, query, log, snapshot, index);
            }
            else if (wID % CHECK_WARP == 1)
            {
                check_0_1(NO, query, log, snapshot, index);
            }
            else if (wID % CHECK_WARP == 2)
            {
                check_1_0(NO, query, log, snapshot, index);
            }
            else if (wID % CHECK_WARP == 3)
            {
                check_2_0(NO, query, log, snapshot, index);
            }
            else if (wID % CHECK_WARP == 4)
            {
                check_2_1(NO, query, log, snapshot, index);
            }
            else if (wID % CHECK_WARP == 5)
            {
                check_3_0(NO, query, log, snapshot, index);
            }
            else if (wID % CHECK_WARP == 6)
            {
                check_3_1(NO, query, log, snapshot, index);
            }
            else if (wID % CHECK_WARP == 5)
            {
                check_4_0(NO, query, log, snapshot, index);
            }
            else if (wID % CHECK_WARP == 6)
            {
                check_5_0(NO, query, log, snapshot, index);
            }
            else if (wID % CHECK_WARP == 7)
            {
                check_5_1(NO, query, log, snapshot, index);
            }
        }
    }
...
namespace STOCKLEVELQUERY_NAMESPACE
{
    __device__ void op_0_0(unsigned int NO,
                           ...
                           INDEX *index)
    { // select stock
        unsigned int WID = query->query[NO].W_ID;
        unsigned int S_I_ID = query->query[NO].S_ID;
        unsigned int TID = __ldg(&query->TID[NO]);
        unsigned int Loc = WID * 100000 + S_I_ID;
        unsigned int log_loc_up = (STOCK_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + READ_TYPE;
        query->set_Loc[STOCK_CNT * 0 + NO] = Loc;
        __REGISTER(0, NO, 0, STOCK_OP_CNT, TID, Loc, log_loc_up, log_loc_down, log);
    }
    __device__ void op_0_1(unsigned int NO,
                           STOCK_SET *query,
                           LOG *log,
    ...
    { // select stock
        ...
        unsigned int S_I_ID = query->query[NO].S_ID;
        unsigned int Loc = WID * 100000 + S_I_ID;
        __READ(NO, 0, 2, Loc, STOCK_CNT, STOCK_SIZE, STOCK_COLUMN, 0, query->set_local_set, snapshot->stock_snapshot);
    }
    __device__ void execute(unsigned int NO,
                            STOCK_SET *query,
                            LOG *log,
    ...
    {
        unsigned int TID = __ldg(&query->TID[NO]);
        unsigned int Loc = __ldg(&query->set_Loc[STOCK_CNT * 0 + NO]);
        unsigned int log_loc_up = (STOCK_ID << 24) + (Loc >> 8);
        unsigned int log_loc_down = (Loc << 24) + WRITE_TYPE;
        __CHECK(0, NO, Loc, TID, log_loc_up, log_loc_down, query->raw, log);
    }
